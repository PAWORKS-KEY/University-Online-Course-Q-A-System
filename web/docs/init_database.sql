...
-- 使用数据库（根据 application.properties 中的配置）
USE webcourse;
...
-- 2. 插入用户数据
-- ============================================
-- 密码：所有测试账号密码统一为 "123456"（明文存储，因为使用 NoOpPasswordEncoder）
...
-- 注意事项
-- ============================================
-- 1. 本脚本使用MySQL变量动态获取ID，确保外键关系正确
-- 2. 文件路径是示例路径，实际文件需要手动上传到服务器
-- 3. 密码使用明文存储（因为使用NoOpPasswordEncoder），生产环境请使用加密存储
-- 4. 执行脚本前请确保数据库已创建（webcourse）
-- 5. 如果表已存在数据，脚本会先清空，请谨慎使用
-- 6. 执行方式：在MySQL客户端中执行此脚本，或使用命令行：mysql -u root -p webcourse < init_database_with_test_data.sql
